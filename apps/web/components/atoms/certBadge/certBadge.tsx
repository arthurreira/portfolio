...
export function CertBadge({ tone, label, sub, glyph, imgSrc, onActivate, size, mounted, index }: CertBadgeProps & { mounted: boolean; index: number }) {
      ...
      >
        <div style={{ filter: `drop-shadow(0 0 ${hot || pop ? 16 : 4}px ${c.glow})`, transition: 'filter .2s ease' }}>
          <svg role="img" aria-label={label} viewBox="0 0 100 100" width={s.svg} height={s.svg}>
            <title>{label}</title>
            {imgSrc
              ? <image href={imgSrc} x="10" y="10" width="80" height="80" preserveAspectRatio="xMidYMid meet" />
